...
function WaitTimeQueuePreview() {
  const laneLabels = ["Quai 1", "Quai 2", "Quai 3", "Quai 4", "Quai 5", "Quai 6"]
  const targetMinutes = [14, 19, 61, 22, 17, 26]
  const criticalLane = 2

  const [minutes, setMinutes] = useState<number[]>(() => laneLabels.map(() => 4))
  const [showRecommendation, setShowRecommendation] = useState(false)
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      setMinutes(targetMinutes.map((target) => Math.round(4 + (target - 4) * progress)))
  ...
  function laneStatus(mins: number) {
    if (mins >= 45) return "blocked" as const
    if (mins >= 25) return "watch" as const
    return "ok" as const
  }
  ...
  return (
    <div className="mt-5 overflow-hidden rounded-2xl border border-border bg-background">
      <div className="flex items-center justify-between border-b border-border px-4 py-3">
        <div>
          <p className="text-xs font-semibold text-foreground">Aperçu, Temps d&apos;attente par quai</p>
          <p className="text-[11px] text-muted-foreground">Exemple avec vos futures données</p>
        </div>
      ...
      <div className="space-y-2.5 p-4">
        {laneLabels.map((label, i) => {
          ...
          const status = laneStatus(mins)
          const widthPercent = Math.min(100, (mins / 65) * 100)
              ...
              <div className="h-6 flex-1 overflow-hidden rounded-full bg-muted">
                <div
                  className={cn(
                    ...
                    status === "ok" && "bg-emerald-500/50",
                    status === "watch" && "bg-amber-500/60",
      ...
      >
        <span className="whitespace-nowrap font-mono text-[11px] text-muted-foreground">
          {laneLabels[criticalLane]}
        </span>
        <p className="text-xs leading-5 text-foreground">
          Temps d&apos;attente à <span className="font-semibold text-red-600">{targetMinutes[criticalLane]} min</span>,
          soit 3x la moyenne des autres quais. Réaffecter un poste de chargement absorberait le pic avant
          qu&apos;il ne s&apos;aggrave.
        </p>
      </div>
    </div>
        ...
        <div className="flex flex-col gap-8">
          ...
          {step === 2 && sector && (
            <div>
              ...
              {sector === "logistics" && selectedEquipment.includes("wait") && <WaitTimeQueuePreview />}
